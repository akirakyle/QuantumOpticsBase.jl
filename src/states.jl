...
"""
mutable struct Bra{B,T} <: AbstractBra{B}
    basis::B
    data::T
...
"""
mutable struct Ket{B,T} <: AbstractKet{B}
    basis::B
    data::T
...
==(x::Ket{B}, y::Ket{B}) where {B} = (x.data==y.data)
==(x::Bra{B}, y::Bra{B}) where {B} = (x.data==y.data)
==(x::Ket, y::Ket) = false
==(x::Bra, y::Bra) = false

Base.isapprox(x::Ket{B}, y::Ket{B}; kwargs...) where {B} = (isapprox(x.data,y.data;kwargs...))
Base.isapprox(x::Bra{B}, y::Bra{B}; kwargs...) where {B} = (isapprox(x.data,y.data;kwargs...))
...
function permutesystems(state::Ket{B,T}, perm) where {B<:CompositeBasis,T}
    @assert nsubsystems(state.basis) == length(perm)
    @assert isperm(perm)
    data = reshape(state.data, length.(bases(state.basis))...)
    data = permutedims(data, perm)
    data = reshape(data, length(data))
    Ket(permutesystems(state.basis, perm), data)
end
function permutesystems(state::Bra{B,T}, perm) where {B<:CompositeBasis,T}
    @assert nsubsystems(state.basis) == length(perm)
    @assert isperm(perm)
    data = reshape(state.data, length.(bases(state.basis))...)
    data = permutedims(data, perm)
...
For a composite system `index` must be a tuple or vector which then creates a tensor
product state ``|i_1⟩⊗|i_2⟩⊗…⊗|i_n⟩`` of the corresponding basis states.
"""
function basisstate(::Type{T}, b::CompositeBasis, indices) where T
    @assert nsubsystems(b) == length(indices)
    x = zeros(T, length(b))
    x[LinearIndices(length.(bases(b)))[indices...]] = one(T)
    Ket(b, x)
end
...
"""
function sparsebasisstate(::Type{T}, b::CompositeBasis, indices) where T
    @assert nsubsystems(b) == length(indices)
