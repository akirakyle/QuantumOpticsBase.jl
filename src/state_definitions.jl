...
"""
function coherentthermalstate(::Type{C},basis::B,H::AbstractOperator{KetBraBasis{B,B}},T,alpha) where {C,B<:FockBasis}
    D = displace(C,basis,alpha)
    return D*thermalstate(H,T)*dagger(D)
end
coherentthermalstate(basis::B,H::AbstractOperator{KetBraBasis{B,B}},T,alpha) where B<:FockBasis = coherentthermalstate(ComplexF64,basis,H,T,alpha)
